...
pub fn authenticate(app, scopes) {
  ...
  let url = auth_url(client_id, redirect_uri, state, scopes)
  ...
  use Nil <- t.try(case string.lowercase(token_type) == "bearer" {
    True -> Ok(Nil)
    False ->
      Error(snag.new("returned token_type was not 'Bearer'" <> token_type))
  })
  t.Done(access_token)
}

fn auth_url(client_id, redirect_uri, state, scopes) {
  let query = [
    #("client_id", client_id),
    #("response_type", "token"),
