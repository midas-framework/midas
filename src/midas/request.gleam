...
pub fn host(request: Request) -> String {
  let Request(method: _, authority: authority, headers: _, path: _, body: _) = request
...
pub fn segments(request: Request) -> List(String) {
  let Request(method: _, authority: _, headers: _, path: raw_path, body: _) = request
...
pub fn set_header(request: Request, key: String, value: String) -> Request {
    let Request(method: method, authority: authority, headers: headers, path: path, body: body) = request
    let headers = h_utils.append(headers, key, value)
    Request(method: method, authority: authority, headers: headers, path: path, body: body)
}


pub fn get_header(request: Request, key: String) -> Option(String) {
    let Request(method: _, authority: _, headers: headers, path: _, body: _) = request
