...
pub fn parse_segments_test() {
  let request = Request(method: http.Get, authority: "e.test", headers: [], path: "/", body: "")
  expect.equal(segments(request), [])

  let request = Request(method: http.Get, authority: "e.test", headers: [], path: "/foo/bar", body: "")
  ...
  let request = Request(method: http.Get, authority: "e.test", headers: [], path: "/foo//bar?baz=5", body: "")
  expect.equal(segments(request), ["foo", "bar"])

  let request = Request(method: http.Get, authority: "e.test", headers: [], path: "/?baz=5", body: "")
  expect.equal(segments(request), [])
}
...
pub fn parse_host_test() {
  ...
  let request = Request(method: http.Get, authority: "e.test:8080", headers: [], path: "/", body: "")
  expect.equal(host(request), "e.test")
}
