...
type Down{
    Down(ref: process.Ref, monitor_type: Atom, pid: process.BarePid, reason: Dynamic)
}
type Exit{
...
pub fn listing_children_test() {
  ...
  assert Ok(Down(ref: ref, pid: pid, reason: reason, ..)) = unsafe_receive(Milliseconds(100))
  should.equal(ref, r1)
  ...
  should.equal(dynamic.element(reason, 0), Ok(dynamic.from(tuple(atom.create_from_string("badmatch"), Ok(Crash)))))
